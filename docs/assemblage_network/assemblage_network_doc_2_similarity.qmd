...
I've introduced you to a couple archaeological concepts now, in addition to a few concepts about sets and graphs. Now I'd like to pause to explain why we're so interested in finding these subsets and partitions. You see, archaeologists don't really interpret artifacts *per se*.
...
Just think for a moment about what you're trash might show about *your* daily behavior!
...
For example, does an identified assemblage of artifacts have a specific range of dates so that it uniquely identifies *when* or *how long* a site was occupied? Does one assemblage relate to domestic activities while another reflects economic activities, and are those spaces (i.e., their contexts) separate or comingled? If the site was occupied multiple times or by multiple households, were they continuous or intermittent? Different assemblages might be mixed into the same context if they were continuous.
...
$$
Overlap(X,Y) = \frac{|\{b,c\}|}{\min(|\{a, b, c\}|, |\{b,c, d, e, f\}|)}=\frac{2}{\min (3, 5)} = \frac{2}{3} \approx 0.67 
$$

Easy, right? It's simply the proportion of the smaller set's members that are also members of the larger set (thus, *overlap*).

Notice, though, that if *all* of the elements of the smaller set were in the larger set -- i.e., if $X$ were a complete *subset* of $Y$ -- then the coefficient would still equal $1$ regardless of how many additional members were in $Y$. So, this similarity measure is defining *similarity* in a very specific way. It includes an assumption that any subset is effectively *equivalent* to its parent set. That little quirk of definition can be quite useful, depending on your application.

The overlap coefficient is a common set similarity metric used in everything from gene sequencing to topic modeling. It's also quite easy to calculate across a binary incidence matrix.

```{r}

# Szymkiewicz-Simpson overlap coefficient for binary input matrix for binary
# incidence matrices
